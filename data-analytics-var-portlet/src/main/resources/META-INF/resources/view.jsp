...
function varVisualization(taskid) {
	$.ajax({
		type: "POST",
	    data: {"<portlet:namespace />token" : token, "<portlet:namespace />taskid" : taskid},
        url: "<%=serveResource%>",
   	    success: function (response) {
   	    	$("#var").html('<img src="data:image/png;base64,' + response + '" />');
        }
    });
}
</script>
...
<div class="panel panel-default">
	<div class="panel-body" id="var">
			<p>Select an ensemble task from the Experiment monitoring table to visualise the variance map.</p>
	</div>
</div>
